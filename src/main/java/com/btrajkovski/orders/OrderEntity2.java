...
public class OrderEntity2 extends EventSourcedBehaviorWithEnforcedReplies<OrderEntity2.Command, OrderEntity2.Event, OrderEntity2.State> {
    ...
    public static class OrderSummary implements JsonSerializable {
        ...
        public final String item;
        public final State.OrderStatus state;
        public final Boolean isShippedSuccessfully;

        @JsonCreator
        public OrderSummary(@JsonProperty("id") String id, @JsonProperty("item") String item, @JsonProperty("state") State.OrderStatus state, @JsonProperty("isShippedSuccessfully") Boolean isShippedSuccessfully) {
            this.id = id;
            this.item = item;
            this.state = state;
            this.isShippedSuccessfully = isShippedSuccessfully;
    ...
    public static Behavior<Command> create(String orderId) {
        return Behaviors.setup(ctx -> EventSourcedBehavior.start(new OrderEntity2(orderId, ctx), ctx));
    }
    ...
    private OrderEntity2(String orderId, ActorContext<Command> ctx) {
    ...
    public CommandHandlerWithReply<Command, Event, State> commandHandler() {
        ...
        eventsBuilders.forState(state -> state.status == State.OrderStatus.CREATED)
                .onCommand(PayOrder.class, this::onPayOrder);

        eventsBuilders.forState(state -> state.status == State.OrderStatus.PAID)
                .onCommand(OrderInFulfilment.class, this::onOrderInFulfilment);

        eventsBuilders.forState(state -> state.status == State.OrderStatus.IN_FULFILLMENT)
    ...
    public EventHandler<State, Event> eventHandler() {
        return newEventHandlerBuilder()
                .forAnyState()
                .onEvent(OrderCreated.class, (state, event) -> new State(event.item, State.OrderStatus.CREATED, null))
    ...
    private ReplyEffect<Event, State> payNotAllowed(State state, PayOrder command) {
        context.getLog().info("Command not allowed");
    ...
    private ReplyEffect<Event, State> onPayOrder(PayOrder command) {
        ...
        ActorRef<FulfilmentProvider2.Command> fulfilmentProvider = context.spawn(FulfilmentProvider2.create(), "fulfilment-provider-" + orderId);

        return Effect()
                .persist(new OrderPaid(orderId))
                .thenRun(() -> fulfilmentProvider.tell(new FulfilmentProvider2.ShipOrder(orderId, context.getSelf())))
                .thenReply(command.replyTo, newState -> StatusReply.success(newState.toSummary(orderId)));
    }
